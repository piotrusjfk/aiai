...
def czy_rzut_ponowny():
    odp = input("Czy chcesz rzucić jeszcze raz? (t/n)")
    if odp == "t" or odp == "T":
        return True
    else:
        return False
...
for tura in range(13):
    ...
    pokaz_kosci()
    for i in range(2):
        rzut_ponowny = czy_rzut_ponowny()
        if rzut_ponowny:
            kosci_do_rzutu = input("Wpisz numery do przelosowania (bez spacji): ")
            rzuc_koscmi(kosci_do_rzutu)
            pokaz_kosci()
        else:
            break
    pokaz_tabele_punktow()
